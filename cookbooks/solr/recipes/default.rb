...
remote_file node.solr.download do
  ...
  mode     0644
  not_if   "test -f #{node.solr.download}"
end

bash 'unpack solr' do
  code   "tar xzf #{node.solr.download} -C #{node.solr.directory}"
  not_if { File.exists? node.solr.download }
end
...
directory node.solr.share do
    owner node.jetty.user
    group node.jetty.group
    mode  '755'
end

bash 'Copy contrib files' do
  code   "cp -R #{node.solr.extracted}/contrib #{node.solr.share}"
  not_if "test -d #{node.solr.share}/contrib"
  notifies :restart, resources(:service => 'jetty')
end

bash 'Copy dist files' do
  code   "cp -R #{node.solr.extracted}/dist #{node.solr.share}"
  not_if "test -d #{node.solr.share}/dist"
  notifies :restart, resources(:service => 'jetty')
end
